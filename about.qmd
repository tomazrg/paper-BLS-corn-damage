...
median(simulated_data_lmer$sev)
...
```
```{r}
simulated_data_lmer$hybrid2 <- rep(paste0("hybrid_", sprintf("%02d", 1:50)), times = 200)

# Número total de linhas do dataframe
num_linhas <- nrow(simulated_data_lmer)

# Criar a sequência que repete cada "hybrid_X" 50 vezes e recicla conforme o tamanho do dataframe
simulated_data_lmer$hybrid2 <- rep(paste0("hybrid_", sprintf("%02d", 1:50)), each = 50, length.out = num_linhas)

lmer_stats <- simulated_data_lmer %>%
  dplyr::select(experiment, yld, sev,hybrid2) %>%
  dplyr::group_by(hybrid2) %>%
  do({
    model <- lmer(yld ~ sev + (1 | experiment), data = .)  # Modelo misto com efeito aleatório
    tidy_model <- tidy(model)  # Resumo do modelo
    confint_model <- confint(model) %>% as.data.frame()  # Intervalos de confiança
    confint_model <- tibble::rownames_to_column(confint_model, "term")  # Ajuste os nomes das colunas
    left_join(tidy_model, confint_model, by = "term")  # Junta os resultados
  })
```
...
sev_lmer <- lmer(mean_yld ~ mean_sev + (1 | trial) + (1 | hybrid), data = estria2)
sev_lmer <- lmer(mean_yld ~ mean_sev + (1 | hybrid), data = estria2)


performance::check_normality(sev_lmer)
performance::check_heteroscedasticity(sev_lmer)
performance::check_autocorrelation(sev_lmer)
performance::check_model(sev_lmer)
sim_resids <- DHARMa::simulateResiduals(fittedModel = sev_lmer)
plot(sim_resids)
test_resids <- DHARMa::testResiduals(sim_resids)
print(test_resids)
simu_model_lmer
```
...
# Ajusta o modelo com os dados observados
obs_model_lmer <- lmer(mean_yld ~ mean_sev + (1 | trial), data = estria2)

# Obtém os parâmetros do modelo real
fixed_intercept <- fixef(obs_model_lmer)[1]
fixed_slope <- fixef(obs_model_lmer)[2]
random_effect_sd <- sqrt(as.numeric(VarCorr(obs_model_lmer)$trial[1])) # Variância dos efeitos aleatórios
residual_sd_obs <- sd(resid(obs_model_lmer)) # Desvio padrão dos resíduos

# Número de experimentos e pontos por experimento
...
for (exp in 1:num_experiments) {
  ...
  # Simula efeito aleatório do experimento
  u_j <- rnorm(1, mean = 0, sd = random_effect_sd)
  
  # Simula severidade com a mesma distribuição dos dados reais
  sev <- sample(estria2$mean_sev, size = points_per_experiment, replace = F)
  
  # Simula resíduos com variância ajustada
  residuals <- rnorm(points_per_experiment, mean = 0, sd = residual_sd_obs)
...
# Ajusta o modelo com os dados simulados
simu_model_lmer <- lmer(yld ~ sev + (1 | experiment), data = simulated_data_lmer)

# Comparação das variâncias
var_obs_yld <- var(estria2$mean_yld)
var_sim_yld <- var(simulated_data_lmer$yld)

cat("Variância do rendimento (observado):", var_obs_yld, "\n")
cat("Variância do rendimento (simulado):", var_sim_yld, "\n")
...
curve_sev_lmer = powerCurve(ext_sev_lmer, along = "mean_sev", nsim = 100,breaks = 1:100)

curve_sev_lmer2 = as.data.frame(summary(curve_sev_lmer))
...
brusone = read_xlsx("data/brusone.xlsx")
...
ma_c2 <- ma_c |> 
  filter(!is.na(sev), !is.na(yld)) |>  # Remove NAs corretamente
  group_by(study,cultivar) |> 
  summarise(mean_sev = mean(sev),  # Evita problemas com NAs restantes
            mean_yld = mean(yld))

ma_c2$study = as.factor(ma_c2$study)
lmer_ts = lmer(mean_yld ~ mean_sev + (1|state), data = ma_c2)

ext_sev_lmer = extend(lmer_ts, along = "mean_sev", n=100)

curve_sev_lmer = powerCurve(ext_sev_lmer, along = "mean_sev", nsim = 40, breaks = 0:100)

curve_sev_lmer2 = as.data.frame(summary(curve_sev_lmer))

cor(simulated_data_lmer$yld,simulated_data_lmer$sev)
